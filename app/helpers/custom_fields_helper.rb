...
module CustomFieldsHelper
  ...
  def custom_field_label_tag1(name, custom_value, arg, options={})
    required = options[:required] || custom_value.custom_field.is_required?
    title = custom_value.custom_field.description.presence
    content = content_tag 'span', arg, :title => title
  ...
  def custom_field_tag_with_label(name, custom_value, options={})
    ...
    # Return custom field tag with its label tag
  def custom_field_tag_with_label_new(name, custom_value, options={})
    if custom_value.custom_field.name !="督办" && custom_value.custom_field.name!="牵头人" && custom_value.custom_field.name!="督办时点" && custom_value.custom_field.name!="索引"
      custom_field_label_tag(name, custom_value, options) + custom_field_tag(name, custom_value)
    elsif custom_value.custom_field.name == "督办"
      "<input type=\"hidden\" id=\"dubanId\" value=\"#{custom_value.custom_field.id}\" >".html_safe

    elsif custom_value.custom_field.name == "牵头人"
      "<input type=\"hidden\" id=\"qiantourenId\" value=\"#{custom_value.custom_field.id}\" >".html_safe
    elsif custom_value.custom_field.name == "督办时点"
      "<input type=\"hidden\" id=\"dubanshidianId\" value=\"#{custom_value.custom_field.id}\" >".html_safe
    elsif custom_value.custom_field.name == "索引"
      "<input type=\"hidden\" id=\"suoyinId\" value=\"#{custom_value.custom_field.id}\" >".html_safe
      
    end
  end


  def custom_field_tag_with_label_issue(name, custom_value, issue,options={})
      cus_name = custom_value.custom_field.name
      regex1 = "add1_(\w*)"
      if issue.status.name=="未启动"
          if cus_name.match(regex1)
            custom_field_label_tag(name, custom_value, options) + custom_field_tag(name, custom_value)
          end
      else
          custom_field_label_tag(name, custom_value, options) + custom_field_tag(name, custom_value)
  ...
  # Return custom field tag with its label tag
  def custom_field_tag_with_label_project(name, custom_value, options={})
    if custom_value.custom_field.name !="督办" && custom_value.custom_field.name!="牵头人"
      custom_field_label_tag(name, custom_value, options) + custom_field_tag(name, custom_value)
    elsif custom_value.custom_field.name == "督办"
      "".html_safe
