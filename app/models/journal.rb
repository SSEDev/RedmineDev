...
class Journal < ActiveRecord::Base
  ...
  def self.searchJournals(fromTime=Time.now-7.day,toTime=Time.now)
    return Journal.where("created_on < ? AND created_on > ?", toTime,fromTime).all(:order => "created_on desc")
  end
